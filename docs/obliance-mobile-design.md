...
| | |
|---|---|
| Statut | Conception finale v1.0, à valider par le propriétaire du produit |
...
- **Le socle** (authentification, réseau, temps réel, design system, coquille de navigation, garde-fous d'action, notifications, mise à jour, vue web, tunnel, terminal) ne contient aucun type Obliance. Obliview et les autres apps Obli pourront démarrer dessus.
...
| Taille de la v1 | ≈ 31 ps centrées sur la boucle d'astreinte | 44 ps (« flotte »), 42 ps à 3 développeurs (« tablette ») | La boucle alerte → appareil → correction doit arriver tôt. |
...
1. **La nuit, sur téléphone** : « Qu'est-ce qui brûle, chez quel client, et est-ce que ça dure encore ? », puis une correction d'une main, puis « Prévenez-moi quand il revient ».
...
| P4 | **Le contexte est toujours visible** | Le tenant et l'identité de l'appareil (nom, OS, IP) figurent sur chaque surface d'action et dans chaque confirmation : « Redémarrer SRV-AD2 (BASH) ? ». Agir chez le mauvais client est l'erreur la plus coûteuse d'un MSP. |
...
| P10 | **Le socle avant la fonctionnalité** | Tout ce qui ne parle pas d'appareils vit dans `core:*`, sans aucun type Obliance. Une règle CI vérifie le graphe des modules. |

### 1.3 Personas

| Persona | Rôle dans Obliance | Contexte | Tâches principales | Conséquences pour la conception |
|---|---|---|---|---|
| **Karim Benali**, technicien N2 d'astreinte (principal) | Compte Obligate `og_karim.benali`, **administrateur de plateforme**, comme la plupart des techniciens du MSP ; travaille sur le tenant maître Default (vue globale) et bascule vers les tenants clients pour agir | Une semaine sur quatre d'astreinte. Galaxy S23, Gboard AZERTY. Réveillé à 03:12 par « SRV-AD2: Hors ligne ». Parfois passager en voiture sur 4G. | Savoir en quelques secondes si c'est réel et quelle est l'ampleur ; corriger ou escalader en 5 min ; se rendormir en sachant que l'appareil est revenu | Notifications actionnables, contexte d'incident, « Surveiller », thème Nuit, biométrie plutôt que mots de passe, collage du code 2FA |
| **Julien Moreau**, technicien terrain (secondaire) | `og_julien.moreau`, **pas administrateur de plateforme** ; membre de BASH (rôle de tenant avec `devices.manage`), `rw` sur Siège › Serveurs et Siège › Comptabilité, capacités `execute`, `power`, `remote` ; ses actions restreintes passent par une approbation | Galaxy Tab S9 avec clavier et souris Bluetooth, en paysage, en sous-sol avec un réseau faible | PowerShell sur les serveurs, SSH, ObliReach pour aider une comptable, script sur 12 postes, numéros de série, clés BitLocker | Volets liste-détail, raccourcis clavier, sessions persistantes, dock, cache hors ligne |
| **Sophie Martin**, responsable technique du MSP (secondaire) | `og_sophie.martin`, **administratrice de plateforme** sur le tenant maître Default (vue globale) ; seconde approbatrice | Réunions, trajets ; Pixel 9 | Approuver ou refuser les demandes à deux avant expiration, surveiller la santé par tenant, terminer une session distante suspecte | Approbations dans À traiter avec compte à rebours, notification prioritaire, approbation biométrique, vue par tenant |
