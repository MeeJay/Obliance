/**
 * SSO routes — cross-app "foreign user" authentication for Obliance.
 *
 * Endpoints exposed FOR other apps (Bearer auth = shared secret):
 *   GET  /api/sso/validate-token?token=xxx   — validate a switch token generated by Obliance
 *   GET  /api/sso/users                      — list Obliance users (for admin pre-assignment)
 *
 * Endpoints used BY Obliance's own client (session auth):
 ...
 *
 * Endpoint called by Obliance client after arriving from another app (no auth — IS the auth step):
...
// (Obliview, Obliguard, or Oblimap) so all three can call validate-token.
// ─────────────────────────────────────────────────────────────────────────────
async function requireSsoBearer(req: Request, res: Response): Promise<boolean> {
  ...
  const validKeys = [obliviewCfg?.apiKey, obliguardCfg?.apiKey, oblimapCfg?.apiKey]
...
// Obliance client calls this when user clicks a cross-app link.
...
// Called BY another app (Obliview, Obliguard, Oblimap) with Bearer auth.
...
router.get('/validate-token', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Use { success, data: { user } } format — matches what other apps' exchange expects.
    res.json({
      success: true,
      data: {
        user: {
...
// GET /api/sso/users
// Called BY another app (Obliview, Obliguard, Oblimap) with Bearer auth.
// Returns list of local Obliance users for admin pre-assignment of permissions.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/users', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const users = await db('users')
      ...
      .whereNull('foreign_source') // only expose local Obliance users
...
// Called BY Obliance's OWN client after the user arrives from another app.
