...
export const windowsPerformanceRules: ComplianceRule[] = [
  ...
  r('perf-002', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$v=powercfg /query SCHEME_CURRENT SUB_SLEEP STANDBYIDLE; if($v -match \'0x([0-9a-fA-F]+)\'){$h=[convert]::ToInt32($Matches[1],16); if($h -eq 0){\'PASS\'}else{\'FAIL\'}}else{\'FAIL\'}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
  ...
  r('perf-003', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$lines=powercfg /query SCHEME_CURRENT SUB_VIDEO VIDEOIDLE; $found=$false; foreach($l in $lines){if($l -match "0x([0-9a-fA-F]+)"){$found=$true; $h=[convert]::ToInt32($Matches[1],16)}}; if($found -and $h -le 900){"PASS"}else{"FAIL"}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'optional',
  ...
  r('perf-004', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$v=powercfg /query SCHEME_CURRENT SUB_DISK DISKIDLE; if($v -match \'0x([0-9a-fA-F]+)\'){$h=[convert]::ToInt32($Matches[1],16); if($h -eq 0){\'PASS\'}else{\'FAIL\'}}else{\'FAIL\'}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
