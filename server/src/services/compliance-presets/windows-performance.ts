...
export const windowsPerformanceRules: ComplianceRule[] = [
  ...
  r('perf-002', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: `$r=powercfg /query SCHEME_CURRENT 238c9fa8-0aad-41ed-83f4-97be242c8f20 29f6c1db-86da-48c5-9fdb-f2b67b1f44da 2>&1; $acLine=$r | Select-String 'AC Power'; if($acLine -match '0x([0-9a-fA-F]+)'){$h=[convert]::ToInt32($Matches[1],16); if($h -eq 0){'PASS'}else{'FAIL'}}else{'FAIL'}`,
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
  ...
  r('perf-003', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: `$r=powercfg /query SCHEME_CURRENT 7516b95f-f776-4464-8c53-06167f40cc99 3c0bc021-c8a8-4e07-a973-6b14cbcb2b7e 2>&1; $acLine=$r | Select-String 'AC Power'; if($acLine -match '0x([0-9a-fA-F]+)'){$h=[convert]::ToInt32($Matches[1],16); if($h -le 900){'PASS'}else{'FAIL'}}else{'FAIL'}`,
    expected: 'PASS',
    operator: 'eq',
    severity: 'optional',
  ...
  r('perf-004', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: `$r=powercfg /query SCHEME_CURRENT 0012ee47-9041-4b5d-9b77-535fba8b1442 6738e2c4-e8a5-4a42-b16a-e040e769756e 2>&1; $acLine=$r | Select-String 'AC Power'; if($acLine -match '0x([0-9a-fA-F]+)'){$h=[convert]::ToInt32($Matches[1],16); if($h -eq 0){'PASS'}else{'FAIL'}}else{'FAIL'}`,
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
  ...
  r('perf-005', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: `$r=powercfg /query SCHEME_CURRENT 2a737441-1930-4402-8d77-b2bebba308a3 48e6b7a6-50f5-4782-a5d4-53bb8f07e226 2>&1; $acLine=$r | Select-String 'AC Power'; if($acLine -match '0x([0-9a-fA-F]+)'){$h=[convert]::ToInt32($Matches[1],16); if($h -eq 0){'PASS'}else{'FAIL'}}else{'FAIL'}`,
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
    remediationScript: 'powercfg /SETACVALUEINDEX SCHEME_CURRENT 2a737441-1930-4402-8d77-b2bebba308a3 48e6b7a6-50f5-4782-a5d4-53bb8f07e226 0; powercfg /SETACTIVE SCHEME_CURRENT',
  }),
  ...
  r('perf-016', {
    ...
    operator: 'eq',
    severity: 'moderate',
    remediationScript: 'New-Item -Path \'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\System\' -Force -EA SilentlyContinue | Out-Null; Set-ItemProperty -Path \'HKLM:\\SOFTWARE\\Policies\\Microsoft\\Windows\\System\' -Name \'EnableActivityFeed\' -Value 0 -Type DWord -Force',
  }),
  r('perf-017', {
  ...
  r('perf-030', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: `$found=$false; Get-ChildItem 'Registry::HKU' -EA SilentlyContinue | Where-Object { $_.Name -match 'S-1-5-21.*[^_]$' } | ForEach-Object { $p="Registry::$($_.Name)\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\VisualEffects"; $v=(Get-ItemProperty -Path $p -Name VisualFXSetting -EA SilentlyContinue).VisualFXSetting; if($v -eq 2){$found=$true} }; if($found){'PASS'}else{'FAIL'}`,
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
    remediationScript: `Get-ChildItem 'Registry::HKU' -EA SilentlyContinue | Where-Object { $_.Name -match 'S-1-5-21.*[^_]$' } | ForEach-Object { $p="$($_.PSPath)\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\VisualEffects"; New-Item -Path $p -Force -EA SilentlyContinue | Out-Null; Set-ItemProperty -Path $p -Name VisualFXSetting -Value 2 -Type DWord -Force }`,
  }),
  r('perf-031', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: `$found=$true; Get-ChildItem 'Registry::HKU' -EA SilentlyContinue | Where-Object { $_.Name -match 'S-1-5-21.*[^_]$' } | ForEach-Object { $p="Registry::$($_.Name)\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize"; $v=(Get-ItemProperty -Path $p -Name EnableTransparency -EA SilentlyContinue).EnableTransparency; if($v -ne 0){$found=$false} }; if($found){'PASS'}else{'FAIL'}`,
    expected: 'PASS',
    operator: 'eq',
    severity: 'optional',
    remediationScript: `Get-ChildItem 'Registry::HKU' -EA SilentlyContinue | Where-Object { $_.Name -match 'S-1-5-21.*[^_]$' } | ForEach-Object { $p="$($_.PSPath)\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize"; New-Item -Path $p -Force -EA SilentlyContinue | Out-Null; Set-ItemProperty -Path $p -Name EnableTransparency -Value 0 -Type DWord -Force }`,
  }),
  r('perf-032', {
  ...
  r('perf-043', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$s=Get-Service -Name Fax -EA SilentlyContinue; if(!$s -or $s.StartType -eq "Disabled"){"PASS"}else{"FAIL"}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'optional',
    remediationScript: 'if(Get-Service Fax -EA SilentlyContinue){Stop-Service Fax -Force -EA SilentlyContinue; Set-Service Fax -StartupType Disabled}',
  }),
  r('perf-044', {
  ...
  r('perf-071', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$r=(Get-NetTCPSetting -SettingName InternetCustom -EA SilentlyContinue).AutoTuningLevelLocal; if(!$r){$r=(netsh int tcp show global | Select-String "normal")}; if($r -match "normal" -or $r -eq "Normal"){"PASS"}else{"FAIL"}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'moderate',
  ...
  r('perf-072', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$r=netsh int tcp show global; if($r -match ":\\s*disabled" -and $r -match "ECN|ecn"){"PASS"}else{$v=(Get-NetTCPSetting -EA SilentlyContinue).EcnCapability; if($v -eq "Disabled"){"PASS"}else{"FAIL"}}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'optional',
  ...
  r('perf-073', {
    ...
    checkType: 'command',
    targetPlatform: 'windows',
    target: '$r=netsh int tcp show global; if($r -match "Timestamps|horodatage" -and $r -match "disabled|désactivé"){"PASS"}else{$v=(Get-NetTCPSetting -EA SilentlyContinue).Timestamps; if($v -eq "Disabled"){"PASS"}else{"FAIL"}}',
    expected: 'PASS',
    operator: 'eq',
    severity: 'optional',
