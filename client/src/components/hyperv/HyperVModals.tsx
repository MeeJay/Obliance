...
import { Cpu, MemoryStick, Camera, RotateCcw, Trash2, Plus } from 'lucide-react';
...
// Shared Modal (docs/obli-mobile.md §8): full-screen sheet on phones, dvh
// max-height, Escape + Android back. A backdrop tap still closes with a
// mouse; on touch it does not (tapping outside to dismiss the soft keyboard
// must not discard the form).
...
const plainInput = { autoCapitalize: 'off', autoCorrect: 'off', spellCheck: false } as const;
    ...
    <ModalShell title={t('hyperv.createTitle') || 'New virtual machine'} icon={<Plus className="w-4 h-4 text-accent" />} onClose={onClose}>
      <div className="space-y-3">
        <div>
          ...
          <input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder="WEB-01" className={inputCls} {...plainInput} />
        </div>
        ...
        <div>
          ...
          <input value={switchName} onChange={(e) => setSwitchName(e.target.value)} placeholder="Default Switch" className={inputCls} {...plainInput} />
...
export function CheckpointModal({ vm, onClose, run }: { vm: VirtualMachine; onClose: () => void; run: RunAction }) {
  ...
  const act = async (action: VmAction, checkpointName?: string) => {
    if (action === 'checkpoint_delete' && !(await confirm({
      message: t('hyperv.confirmCpDelete', { name: checkpointName, defaultValue: 'Delete checkpoint "{{name}}"?' }),
      danger: true,
    }))) return;
    if (action === 'checkpoint_apply' && !(await confirm({
      message: t('hyperv.confirmCpApply', { name: checkpointName, defaultValue: 'Restore VM to checkpoint "{{name}}"? Unsaved state since then is lost.' }),
      danger: true,
      confirmLabel: t('hyperv.applyCheckpoint') || 'Restore to this checkpoint',
    }))) return;
    ...
    <ModalShell title={`${t('hyperv.checkpointsTitle') || 'Checkpoints'} — ${vm.name}`} icon={<Camera className="w-4 h-4 text-accent" />} onClose={onClose}>
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={t('hyperv.checkpointNamePh') || 'New checkpoint name (optional)'} className={`${inputCls} min-w-0`} />
            ...
            {checkpoints.map((cp) => (
              <div key={cp.name} className="flex items-center gap-2 px-3 py-2">
                ...
                <IconButton
                  ...
                  label={t('hyperv.applyCheckpoint') || 'Restore to this checkpoint'}
                ...
                <IconButton
                  ...
                  label={t('hyperv.action.delete') || 'Delete'}
