...
export function MaintenanceWindowModal({
 ...
 const [editScopeType, setEditScopeType] = useState<MaintenanceScopeType>('device');
 ...
 await maintenanceApi.disableForScope(created.id, d.scopeType as 'group' | 'monitor' | 'agent', d.scopeId);
 }
 }
 }
 onSaved?.();
 ...
 <div className="flex rounded-md overflow-hidden border border-transparent">
 ...
 className="h-3.5 w-3.5 rounded border-transparent bg-bg-tertiary text-accent focus:ring-accent"
 />
 ...
 type="button"
 onClick={onClose}
 className="rounded-md px-4 py-2 text-sm font-medium text-text-secondary bg-bg-tertiary hover:bg-bg-hover border border-transparent transition-colors"
 >
 {t('common.cancel')}
 </button>
 <button
