...
export function ApprovalsPage({ embedded = false }: { embedded?: boolean } = {}) {
 ...
 <button
 onClick={() => load(true)}
 className="p-1.5 rounded text-text-muted hover:text-text-primary hover:border-accent/40 transition-colors"
 title="Refresh"
 >
 ...
 className="flex items-center gap-1 text-xs px-2.5 py-1 rounded text-text-muted hover:text-text-primary disabled:opacity-50"
 >
 Cancel
 </button>
 )}
