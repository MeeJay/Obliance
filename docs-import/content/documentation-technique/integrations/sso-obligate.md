...
Monte vraisemblablement sous le prefixe `/auth` (d'apres les commentaires JSDoc du fichier — a reverifier dans `app.ts` avant de le citer comme chemin absolu garanti).
...
1. Lit la config brute Obligate via `appConfigService.getObligateRaw()` — `raw.apiKey` n'est **jamais** expose au navigateur.
...
4. Genere un parametre anti-CSRF `state` via `crypto.randomBytes(32)` (conforme RFC 6749 §10.12), stocke dans `req.session.oauthState`.
...
2. Valide `state` contre `req.session.oauthState` — usage unique, supprime immediatement apres verification.
...
> Le detail exact de l'echange cote `obligate.service.ts` (endpoint `/token`, duree de vie du code, format precis de l'assertion) n'est pas documente ici — se referer directement au fichier source pour ce niveau de detail.

### 3. Provisioning local — `provisionObligateUser()`

Fonction locale definie dans `obligateCallback.routes.ts` (environ lignes 30-265). A partir de l'`ObligateUserAssertion` recue :

- Si `assertion.role === 'admin'` → mappe vers l'admin global (god-view, tenant master).
...
## Reverse auth — `GET /auth/app-info` (ligne 388)
...
Purge automatique via `setInterval(...).unref()` — pas de persistance DB pour ces requetes transitoires.

## Points a verifier avant extension

- Le montage exact du prefixe (`/auth`) dans `server/src/app.ts` n'a pas ete confirme explicitement dans ce lot de lecture — a valider avant toute documentation externe citant un chemin absolu garanti.
